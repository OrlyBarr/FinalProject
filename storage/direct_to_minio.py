...
def run(dry_run=False, only=None) -> dict:
    ...
    if dry_run:
        ...
        log.info(f"  buses={len(buses)}  trains={len(trains)}  traffic={len(traffic)}")
        for r in (buses[:2] + trains[:2] + traffic[:2]):
            print(json.dumps(r, ensure_ascii=False, indent=2))
        return {"buses": len(buses), "trains": len(trains), "traffic": len(traffic)}
...
if __name__ == "__main__":
    ...
    parser.add_argument("--only", choices=["buses", "trains", "traffic"], help="Fetch only one type")
